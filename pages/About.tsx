
import React from 'react';
import { Target, Users, ShieldCheck, Heart } from 'lucide-react';

const About: React.FC = () => {
  return (
    <div className="py-12">
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-24">
          <div className="relative">
            <img
              src="https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&w=1200&q=80"
              alt="Team working"
              className="rounded-3xl shadow-2xl"
            />
            <div className="absolute -bottom-6 -right-6 w-1/2 aspect-video bg-emerald-600 p-8 rounded-2xl text-white shadow-xl hidden md:block">
              <p className="text-2xl font-bold italic">"Rooted in Excellence, Growing for Sydney."</p>
            </div>
          </div>
          <div>
            <h1 className="text-5xl font-bold text-stone-900 mb-8">GreenSydney: Where Nature Meets Artistry</h1>
            <p className="text-lg text-stone-600 mb-6 leading-relaxed">
              Founded in 2010, GreenSydney Landscapes began with a simple mission: to bring professional, high-end landscape architecture to the residential heart of Sydney.
            </p>
            <p className="text-lg text-stone-600 mb-8 leading-relaxed">
              Our team consists of certified horticulturists, landscape designers, and skilled craftsmen who share a passion for creating outdoor spaces that aren't just gardens, but living ecosystems. We believe that every property has a unique story, and we're here to help you tell it.
            </p>
            <div className="grid grid-cols-2 gap-8">
              <div>
                <h4 className="text-3xl font-bold text-emerald-600">500+</h4>
                <p className="text-stone-500 font-medium uppercase text-xs tracking-wider">Projects Completed</p>
              </div>
              <div>
                <h4 className="text-3xl font-bold text-emerald-600">12</h4>
                <p className="text-stone-500 font-medium uppercase text-xs tracking-wider">Expert Staff</p>
              </div>
            </div>
          </div>
        </div>

        {/* Mission & Values */}
        <div className="bg-stone-100 rounded-[3rem] p-12 md:p-20 mb-24">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-stone-900 mb-4">Our Core Values</h2>
            <p className="text-stone-600">The principles that drive every project we touch.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            {[
              { icon: <Target className="w-10 h-10" />, title: 'Mission', text: 'To elevate Sydney living through sustainable and innovative outdoor design.' },
              { icon: <Heart className="w-10 h-10" />, title: 'Sustainability', text: 'Preserving our planet by using eco-friendly materials and local flora.' },
              { icon: <ShieldCheck className="w-10 h-10" />, title: 'Quality', text: 'No shortcuts. We build to last and maintain to perfection.' },
              { icon: <Users className="w-10 h-10" />, title: 'Community', text: 'Supporting local growers and creating spaces that bring people together.' }
            ].map((value, idx) => (
              <div key={idx} className="text-center">
                <div className="bg-white w-20 h-20 rounded-2xl shadow-sm flex items-center justify-center mx-auto mb-6 text-emerald-600">
                  {value.icon}
                </div>
                <h4 className="text-xl font-bold mb-3">{value.title}</h4>
                <p className="text-stone-500 text-sm leading-relaxed">{value.text}</p>
              </div>
            ))}
          </div>
        </div>
      </section>
    </div>
  );
};

export default About;
